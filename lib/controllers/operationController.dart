    ...
    @Bind.query("page") int page,
      ...
      if(name.isNotEmpty)
      {
        try{
          ...
          if(userOperations.isEmpty) return AppResponse.ok(message: "Операции не найдены!");
  ...
  Future<Response> getOperation(
    ...
    @Bind.path("id") int id) async {
      try{
        ...
        if(userOperation == null) return AppResponse.ok(message: "Операция не найдена!");

        final Operations? userOperation_ = await managedContext.fetchObjectWithID<Operations>(userOperation.operationID);
  ...
  Future<Response> createOperation(
    ...
    @Bind.body() Operations operation) async {
      ...
      try{
        ...
        final Operations? createdOperation = await managedContext.fetchObjectWithID<Operations>(newOperationId);
        createdOperation!.removePropertiesFromBackingMap(["user"]);

        return Response.ok(ModelResponse(data: createdOperation.backing.contents, message: "Операция успешно создана!"));
  ...
  Future<Response> deleteOperation(
    ...
    @Bind.path("id") int id) async {
      try{
        ...
        if(operation == null) return Response.badRequest(body: "Операция не найдена!");

        if(operation.user?.id != uid) return AppResponse.ok(message: "У вас нет доступа к данной опреации!");
  ...
  Future<Response> updateOperation(
    ...
    @Bind.path("id") int id) async {
      try{
        ...
        final currentOperation = await managedContext.fetchObjectWithID<Operations>(id);

        if(user == null) return AppResponse.ok(message: "Пользователь не найден!");
        
        if(user.id != currentOperation?.user?.id) return AppResponse.ok(message: "У вас нет доступа к данной опреации!");
